...
История установки Docker на WB-контроллерах прошла три фазы:

1. **Community-installer.** Docker ставился bash-скриптом
   [`wb-docker-manager.sh`](https://github.com/wirenboard/wb-community/blob/main/scripts/docker-install/wb-docker-manager.sh)
   из репозитория wb-community. Никакого пакета — скрипт сам решал, что и
   куда положить. Невоспроизводимо через стандартные apt-флоу, версия Docker
   фиксировалась на момент написания скрипта.

2. **Мета-пакет `wb-docker`** — первая итерация этого проекта.
   Мета-пакет через apt-зависимости тянул Debian'овский `docker.io`. На
   bullseye это `docker.io 20.10.5` (2021 года) — отстающая версия без
   нового compose-плагина и значимых апдейтов. Плюс: разные релизы Debian
   кладут разные `docker.io` — поведенческий контракт у пользователей
   разъезжается. **От подхода отказались** именно по этим двум причинам.

3. **Перепаковка `docker-ce`** — текущий подход, реализованный в этом
   репозитории. WB контролирует и версию Docker, и поведение установки
   независимо от того, что лежит в Debian, и независимо от cadence публикаций
   Docker Inc.
...
Первая итерация именно так и выглядела — отдельный мета-пакет `wb-docker`,
который через apt-зависимости тянул Docker (тогда — `docker.io`). От этой
формы отказались по двум причинам. Первая — отдельный пакет добавляет
лишний шаг в UX (`apt install wb-docker` вместо `apt install docker-ce`).
Вторая — содержимое мета-пакета это де-факто пара конфигурационных файлов,
никакого собственного кода. Положить эти файлы внутрь `docker-ce` проще,
чем поддерживать отдельную единицу версионирования.
...
Технической защиты от добавления стороннего apt-source в пакете
**нет и не планируется** — это сознательный выбор. Пользовательский apt
contract: оператор сам решает, какие репозитории подключать; маинтейнер
пакета не должен переписывать чужие `sources.list.d`. Документация
описывает риск как «не делайте этого» и оставляет ответственность на
операторе.

### iptables alternatives на свежем контроллере

На части WB-релизов группа альтернатив для `iptables` / `ip6tables`
поставляется **незарегистрированной** — `update-alternatives --query
iptables` возвращает «no alternatives». В этой ситуации простой
`update-alternatives --set iptables /usr/sbin/iptables-legacy` молча
падает: устанавливать нечего.

Поэтому postinst-сниппет сначала проверяет `update-alternatives --query`,
и если группа не зарегистрирована — явно подключает обе кандидатуры
(`iptables-nft` и `iptables-legacy`) через `--install` с приоритетами
(`10` и `20` соответственно), и только потом делает `--set` на legacy.
Без этой обвязки на свежей прошивке Docker NAT не запускался — это
известная грабля из вики WB.
...
Что переживает FIT-обновление прошивки и что нет, при postinst-only:

- **Переживают** (всё, что лежит на отдельном разделе `/mnt/data`):
  пользовательские данные Docker (`/mnt/data/docker/lib`), `daemon.json`
  (`/mnt/data/etc/docker/daemon.json`), state containerd
  (`/mnt/data/var/lib/containerd`).
- **Не переживают** (rootfs-объекты, которые FIT перезаписывает):
  симлинки `/etc/docker` и `/var/lib/containerd`, выбор iptables через
  `update-alternatives` (он живёт под `/etc/alternatives/`).

Следствие: после FIT-обновления Docker запустится без симлинков и без
пина iptables-legacy — пользователь увидит «пустой» Docker (демон
смотрит на rootfs-вые `/var/lib/docker` и `/etc/docker`, а его данные —
на `/mnt/data`) и сломанный NAT для контейнеров. Сейчас лечится
переустановкой пакета (`apt-get install --reinstall docker-ce`), которая
заново отработает postinst-сниппет.

Полное автоматическое восстановление на каждой загрузке — отдельный
поток работ (вариант: oneshot systemd-сервис `Before=docker.service`,
см. ветку `feature/INT-1084-fit-survival`).
...
- **Промоут в WB apt repo.** Сначала в `testing`-suite, по одному `.deb`
  через `pipelines/upload-deb`; после полевой проверки — в `stable`.
  Делается после Jenkins-pipeline'а, потому что ручной upload каждой
  итерации не масштабируется.
